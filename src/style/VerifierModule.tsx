...
export default function VerifierModule() {
  ...
  return (
    <div className="min-h-screen bg-cream text-cacao font-sans">
      {/* NAVBAR PUBLIQUE */}
      <nav className="h-20 border-b border-cacao/10 px-6 flex items-center justify-between">
        ...
        <div className="flex items-center gap-4">
          <span className="text-[10px] font-black uppercase text-cacao/40 bg-cacao/5 px-3 py-1 rounded-full">Explorer Public</span>
        </div>
      </nav>
      ...
      <main className="max-w-4xl mx-auto px-6 py-12">
        ...
        <section className="text-center mb-16">
          ...
          <p className="text-cacao/60 mb-8">Scannez un QR Code ou entrez l'identifiant unique du lot pour voir son parcours blockchain.</p>
          
          <div className="flex flex-col md:flex-row gap-4 max-w-2xl mx-auto">
            <div className="relative flex-1">
              <Search className="absolute left-4 top-1/2 -translate-y-1/2 text-cacao/20" size={20} />
              <input 
                type="text" 
                placeholder="Ex: LOT-TG-2026..."
                className="w-full pl-12 pr-4 py-4 bg-white border border-cacao/10 rounded-2xl outline-none focus:border-gold transition-all font-mono text-sm"
                value={searchId}
        ...
        {lotData && (
          <div className="animate-in fade-in slide-in-from-bottom-8 duration-700 space-y-8">
            ...
            <div className="bg-white rounded-[2.5rem] p-10 border border-cacao/10 shadow-sm">
              <h3 className="font-serif font-black text-xl mb-10 text-cacao">Parcours du Produit</h3>
              <div className="space-y-12 relative before:absolute before:left-[17px] before:top-2 before:bottom-2 before:w-0.5 before:bg-cacao/5">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              <div className="bg-white/60 p-8 rounded-[2rem] border border-cacao/10">
                <h4 className="font-black text-xs uppercase tracking-widest text-cacao/40 mb-6 flex items-center gap-2">
              ...
              <div className="bg-white/60 p-8 rounded-[2rem] border border-cacao/10">
                ...
                <div className="flex flex-wrap gap-2">
                   <button className="bg-white px-4 py-2 rounded-xl text-[10px] font-bold border border-cacao/10 hover:border-gold flex items-center gap-2">
    ...
    <div className="relative pl-12">
      ...
      <div>
        <div className="flex justify-between items-start">
          ...
          <span className="text-[10px] font-medium text-cacao/40">{date}</span>
        </div>
        <p className="text-sm text-cacao/70 mt-1">{desc}</p>
        <p className="text-[10px] font-mono text-cacao/40 mt-2 bg-cacao/5 p-2 rounded-lg inline-block">{details}</p>
      </div>
    </div>
  );
}
...
function BlockchainLink({ label, hash }: any) {
  return (
    <div className="flex items-center justify-between p-3 bg-white/40 rounded-xl border border-cacao/10">
      <span className="text-xs font-bold text-cacao/70">{label}</span>
