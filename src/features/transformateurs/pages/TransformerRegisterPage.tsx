// TODO [SIMULATED] This registration page does not call the backend. Replace with authService.register() + transformateur payload.
// Backend integration needed: POST /auth/register with transformateur payload
import React, { useState } from 'react';
import { useNavigate } from 'react-router-dom';
import { motion, AnimatePresence } from 'framer-motion';
import { 
  ArrowLeft, Upload, FileText, ShieldCheck, AlertCircle, 
  CheckCircle2, Factory, MapPin, Phone, Mail, Globe,
  User, Camera, Eye, EyeOff, Info, Award, Package
} from 'lucide-react';
...
interface TransformerFormData {
  ...
  // Certifications
  certificationBio: File | null;
  certificationCommerceEquitable: File | null;
  certificationOrigine: File | null;
  agrémentMinistériel: File | null;
  photoUsine: File | null;
...
export default function TransformerRegisterPage() {
  ...
  const [formData, setFormData] = useState<TransformerFormData>({
    ...
    commune: '',
    coordonneesGPS: '',
    certificationBio: null,
    certificationCommerceEquitable: null,
    certificationOrigine: null,
  ...
  const validateStep = (step: number): boolean => {
    ...
    switch (step) {
      case 1:
        ...
        if (!formData.numeroAgrément.trim()) newErrors.numeroAgrément = 'Numéro d\'agrément obligatoire';
        if (!formData.anneeCreation) newErrors.anneeCreation = 'Année de création obligatoire';
        if (!formData.capaciteProduction.trim()) newErrors.capaciteProduction = 'Capacité de production obligatoire';
      ...
      case 3:
        if (!formData.certificationBio) newErrors.certificationBio = 'Certification Bio obligatoire';
        if (!formData.agrémentMinistériel) newErrors.agrémentMinistériel = 'Agrément ministériel obligatoire';
        if (!formData.photoUsine) newErrors.photoUsine = 'Photo de l\'usine obligatoire';
  ...
  const handleInputChange = (field: keyof TransformerFormData, value: any) => {
  ...
  const handleFileUpload = (field: keyof TransformerFormData, file: File) => {
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      setTimeout(() => {
        navigate('/transformateur/confirmation-inscription');
      }, 2000);
        ...
        return (
          ...
          >
            <div className="text-center mb-8">
              <Factory className="w-16 h-16 text-cacao mx-auto mb-4" />
              <h2 className="text-2xl font-serif font-black text-cacao mb-2">Informations de l'Entreprise</h2>
              <p className="text-sm text-cacao/60">Renseignez les informations de base de votre entreprise de transformation</p>
            </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              <div>
                ...
                <div className="relative">
                  <Factory className="absolute left-3 top-1/2 -translate-y-1/2 text-cacao/40" size={18} />
                  <input
                    type="text"
                    ...
                    placeholder="Ex: Togo Chocoterie Industrielle"
                  />
              ...
              <div>
                ...
                <div className="relative">
                  ...
                  <input
                    type="text"
                    value={formData.numeroAgrément}
                    onChange={(e) => handleInputChange('numeroAgrément', e.target.value)}
                    className={`w-full bg-white border rounded-xl py-3 pl-10 pr-4 text-sm focus:ring-4 focus:ring-gold/10 focus:border-gold outline-none transition-all ${
                      errors.numeroAgrément ? 'border-red-300' : 'border-cacao/10'
                    }`}
                    placeholder="N° d'agrément ministériel"
                  />
                  {errors.numeroAgrément && (
                    <p className="text-red-500 text-xs mt-1 flex items-center gap-1">
                      <AlertCircle size={12} /> {errors.numeroAgrément}
                    </p>
                  )}
                </div>
              </div>
              ...
              <div>
                <label className="block text-sm font-bold text-cacao mb-2">
                  Capacité de production (tonnes/an) <span className="text-red-500">*</span>
                </label>
                <div className="relative">
                  ...
                  <input
                    ...
                    className={`w-full bg-white border rounded-xl py-3 pl-10 pr-4 text-sm focus:ring-4 focus:ring-gold/10 focus:border-gold outline-none transition-all ${
                      errors.capaciteProduction ? 'border-red-300' : 'border-cacao/10'
                    }`}
                    placeholder="Ex: 5000"
                  />
                  {errors.capaciteProduction && (
                    <p className="text-red-500 text-xs mt-1 flex items-center gap-1">
                      <AlertCircle size={12} /> {errors.capaciteProduction}
                    </p>
                  )}
                </div>
              </div>
        ...
        return (
          ...
          >
            <div className="text-center mb-8">
              ...
              <p className="text-sm text-cacao/60">Précisez l'adresse de votre usine de transformation</p>
            </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              <div className="md:col-span-2">
                ...
                <input
                  ...
                  placeholder="Adresse complète de l'usine"
                />
                {errors.siegeSocial && (
                  <p className="text-red-500 text-xs mt-1 flex items-center gap-1">
            ...
            <div className="space-y-6">
              ...
              <div className="bg-emerald-5 rounded-2xl p-6">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  <div>
                    <label className="block text-sm font-bold text-cacao mb-2">
                      Certification Bio <span className="text-red-500">*</span>
                    </label>
                    <div className={`border-2 border-dashed rounded-xl p-4 text-center transition-all cursor-pointer hover:border-gold ${
                      errors.certificationBio ? 'border-red-300' : 'border-cacao/20'
                    }`}>
                      <input
                        type="file"
                        accept=".pdf,.jpg,.jpeg,.png"
                        onChange={(e) => e.target.files?.[0] && handleFileUpload('certificationBio', e.target.files[0])}
                        className="hidden"
                        id="certificationBio"
                      />
                      <label htmlFor="certificationBio" className="cursor-pointer">
                        {formData.certificationBio ? (
                          <div className="flex items-center justify-center gap-2">
                            <CheckCircle2 className="text-emerald-600" size={20} />
                            <span className="text-sm text-cacao">{formData.certificationBio.name}</span>
                          </div>
                        ) : (
                          <div>
                    ...
                    {errors.certificationBio && (
                      <p className="text-red-500 text-xs mt-1 flex items-center gap-1">
                        <AlertCircle size={12} /> {errors.certificationBio}
                      </p>
                    )}
                  </div>
                  ...
                  <div>
                    ...
                    <div className="border-2 border-dashed rounded-xl p-4 text-center transition-all cursor-pointer hover:border-gold border-cacao/20">
                      <input
                        type="file"
                        accept=".pdf,.jpg,.jpeg,.png"
                        onChange={(e) => e.target.files?.[0] && handleFileUpload('certificationCommerceEquitable', e.target.files[0])}
                        className="hidden"
                        id="certificationCommerceEquitable"
                      />
                      <label htmlFor="certificationCommerceEquitable" className="cursor-pointer">
                        {formData.certificationCommerceEquitable ? (
                          <div className="flex items-center justify-center gap-2">
                            <CheckCircle2 className="text-emerald-600" size={20} />
                            <span className="text-sm text-cacao">{formData.certificationCommerceEquitable.name}</span>
                          </div>
                        ) : (
                          <div>
                            <Upload className="w-8 h-8 text-cacao/40 mx-auto mb-2" />
                  ...
                  <div>
                    ...
                    <div className="border-2 border-dashed rounded-xl p-4 text-center transition-all cursor-pointer hover:border-gold border-cacao/20">
                      <input
                        type="file"
                        accept=".pdf,.jpg,.jpeg,.png"
                        onChange={(e) => e.target.files?.[0] && handleFileUpload('certificationOrigine', e.target.files[0])}
                        className="hidden"
                        id="certificationOrigine"
                      />
                      <label htmlFor="certificationOrigine" className="cursor-pointer">
                        {formData.certificationOrigine ? (
                          <div className="flex items-center justify-center gap-2">
                            <CheckCircle2 className="text-emerald-600" size={20} />
                            <span className="text-sm text-cacao">{formData.certificationOrigine.name}</span>
                          </div>
                        ) : (
                          <div>
                            <Upload className="w-8 h-8 text-cacao/40 mx-auto mb-2" />
                  ...
                  <div>
                    <label className="block text-sm font-bold text-cacao mb-2">
                      Photo de l'usine <span className="text-red-500">*</span>
                    </label>
                    <div className={`border-2 border-dashed rounded-xl p-4 text-center transition-all cursor-pointer hover:border-gold ${
                      errors.photoUsine ? 'border-red-300' : 'border-cacao/20'
                    }`}>
                      <input
                        type="file"
                        accept=".jpg,.jpeg,.png"
                        onChange={(e) => e.target.files?.[0] && handleFileUpload('photoUsine', e.target.files[0])}
                        className="hidden"
                        id="photo"
                      />
                      <label htmlFor="photo" className="cursor-pointer">
                        {formData.photoUsine ? (
                          <div className="flex items-center justify-center gap-2">
                            <CheckCircle2 className="text-emerald-600" size={20} />
                            <span className="text-sm text-cacao">{formData.photoUsine.name}</span>
                          </div>
                        ) : (
                          <div>
                    ...
                    {errors.photoUsine && (
                      <p className="text-red-500 text-xs mt-1 flex items-center gap-1">
                        <AlertCircle size={12} /> {errors.photoUsine}
                      </p>
                    )}
                  </div>
                </div>
          ...
          >
            ...
            <div className="bg-cacao/5 rounded-2xl p-6 space-y-4">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4 text-sm">
                ...
                <div>
                  <span className="font-bold text-cacao/60">Capacité:</span>
                  <p className="text-cacao">{formData.capaciteProduction} tonnes/an</p>
                </div>
                <div>
          ...
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-4">
              <motion.button
                ...
                whileTap={{ scale: 0.95 }}
                onClick={() => navigate('/transformateur')}
                className="p-2.5 bg-white shadow-sm border border-cacao/10 rounded-xl text-cacao hover:text-gold transition-all"
              >
                <ArrowLeft size={20} />
              </motion.button>
              <div>
                <h1 className="text-xl font-serif font-black text-cacao tracking-tight">Inscription Transformateur</h1>
