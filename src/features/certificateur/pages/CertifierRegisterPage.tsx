...
import { 
  ...
  CheckCircle2, Award, MapPin, Phone, Mail, Globe,
  User, Camera, Eye, EyeOff, Info, Briefcase,
  GraduationCap, Building2
} from 'lucide-react';
import { toast } from 'react-hot-toast';

interface CertifierFormData {
  ...
  // Certifications
  accréditationOfficielle: File | null;
  certificatCompétences: File | null;
  agrémentMinistériel: File | null;
  photoBureau: File | null;
...
export default function CertifierRegisterPage() {
  ...
  const [formData, setFormData] = useState<CertifierFormData>({
    nomOrganisme: '',
    sigle: '',
    numeroAccreditation: '',
    anneeCreation: '',
    ...
    commune: '',
    coordonneesGPS: '',
    accréditationOfficielle: null,
    certificatCompétences: null,
    agrémentMinistériel: null,
    photoBureau: null,
    walletAddress: '',
  ...
  const typesOrganismes = [
    'Organisme de certification public',
    'Organisme de certification privé',
  ...
  const validateStep = (step: number): boolean => {
    ...
    switch (step) {
      case 1:
        if (!formData.nomOrganisme.trim()) newErrors.nomOrganisme = 'Nom obligatoire';
        if (!formData.sigle.trim()) newErrors.sigle = 'Sigle obligatoire';
        if (!formData.numeroAccreditation.trim()) newErrors.numeroAccreditation = 'Numéro d\'accréditation obligatoire';
        if (!formData.anneeCreation) newErrors.anneeCreation = 'Année de création obligatoire';
        if (!formData.typeOrganisme) newErrors.typeOrganisme = 'Type d\'organisme obligatoire';
      ...
      case 3:
        if (!formData.accréditationOfficielle) newErrors.accréditationOfficielle = 'Accréditation officielle obligatoire';
        if (!formData.certificatCompétences) newErrors.certificatCompétences = 'Certificat de compétences obligatoire';
        if (!formData.agrémentMinistériel) newErrors.agrémentMinistériel = 'Agrément ministériel obligatoire';
        if (!formData.photoBureau) newErrors.photoBureau = 'Photo du bureau obligatoire';
  ...
  const handleInputChange = (field: keyof CertifierFormData, value: any) => {
  ...
  const handleFileUpload = (field: keyof CertifierFormData, file: File) => {
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      setTimeout(() => {
        navigate('/certifieur/confirmation-inscription');
      }, 2000);
        ...
        return (
          ...
          >
            <div className="text-center mb-8">
              <Award className="w-16 h-16 text-cacao mx-auto mb-4" />
              <h2 className="text-2xl font-serif font-black text-cacao mb-2">Informations de l'Organisme</h2>
              <p className="text-sm text-cacao/60">Renseignez les informations de base de votre organisme de certification</p>
            </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              <div>
                ...
                <div className="relative">
                  ...
                  <input
                    ...
                    onChange={(e) => handleInputChange('nomOrganisme', e.target.value)}
                    className={`w-full bg-white border rounded-xl py-3 pl-10 pr-4 text-sm focus:ring-4 focus:ring-gold/10 focus:border-gold outline-none transition-all ${
                      errors.nomOrganisme ? 'border-red-300' : 'border-cacao/10'
                    }`}
                    placeholder="Ex: Bureau de Certification Togolais"
                  />
                  {errors.nomOrganisme && (
                    <p className="text-red-500 text-xs mt-1 flex items-center gap-1">
                      <AlertCircle size={12} /> {errors.nomOrganisme}
                    </p>
                  )}
                </div>
              </div>
              ...
              <div>
                <label className="block text-sm font-bold text-cacao mb-2">
                  Numéro d'accréditation <span className="text-red-500">*</span>
                </label>
                <div className="relative">
                  <ShieldCheck className="absolute left-3 top-1/2 -translate-y-1/2 text-cacao/40" size={18} />
                  <input
                    type="text"
                    value={formData.numeroAccreditation}
                    onChange={(e) => handleInputChange('numeroAccreditation', e.target.value)}
                    className={`w-full bg-white border rounded-xl py-3 pl-10 pr-4 text-sm focus:ring-4 focus:ring-gold/10 focus:border-gold outline-none transition-all ${
                      errors.numeroAccreditation ? 'border-red-300' : 'border-cacao/10'
                    }`}
                    placeholder="N° d'accréditation officiel"
                  />
                  {errors.numeroAccreditation && (
                    <p className="text-red-500 text-xs mt-1 flex items-center gap-1">
                      <AlertCircle size={12} /> {errors.numeroAccreditation}
                    </p>
                  )}
                </div>
              </div>
              ...
              <div>
                ...
                <select
                  ...
                  onChange={(e) => handleInputChange('typeOrganisme', e.target.value)}
                  className={`w-full bg-white border rounded-xl py-3 px-4 text-sm focus:ring-4 focus:ring-gold/10 focus:border-gold outline-none transition-all ${
                    errors.typeOrganisme ? 'border-red-300' : 'border-cacao/10'
                  }`}
                >
                ...
                {errors.typeOrganisme && (
                  <p className="text-red-500 text-xs mt-1 flex items-center gap-1">
                    <AlertCircle size={12} /> {errors.typeOrganisme}
                  </p>
                )}
              </div>
              ...
              <div>
                ...
                <div className="relative">
                  ...
                  <input
                    ...
                    placeholder="contact@organisme.tg"
                  />
                  {errors.email && (
                    <p className="text-red-500 text-xs mt-1 flex items-center gap-1">
              ...
              <div>
                ...
                <div className="relative">
                  ...
                  <input
                    ...
                    placeholder="https://www.organisme.tg"
                  />
                </div>
              </div>
            </div>
        ...
        return (
          ...
          >
            <div className="text-center mb-8">
              ...
              <p className="text-sm text-cacao/60">Précisez l'adresse de votre organisme</p>
            </div>
        ...
        return (
          ...
          >
            <div className="text-center mb-8">
              ...
              <p className="text-sm text-cacao/60">Téléchargez vos accréditations et configurez votre wallet</p>
            </div>
            ...
            <div className="space-y-6">
              ...
              <div className="bg-emerald-50 rounded-2xl p-6">
                <h3 className="font-bold text-cacao mb-4 flex items-center gap-2">
                  ...
                  Accréditations (obligatoires)
                </h3>
                
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  <div>
                    <label className="block text-sm font-bold text-cacao mb-2">
                      Accréditation officielle <span className="text-red-500">*</span>
                    </label>
                    <div className={`border-2 border-dashed rounded-xl p-4 text-center transition-all cursor-pointer hover:border-gold ${
                      errors.accréditationOfficielle ? 'border-red-300' : 'border-cacao/20'
                    }`}>
                      <input
                        type="file"
                        accept=".pdf,.jpg,.jpeg,.png"
                        onChange={(e) => e.target.files?.[0] && handleFileUpload('accréditationOfficielle', e.target.files[0])}
                        className="hidden"
                        id="accreditation"
                      />
                      <label htmlFor="accreditation" className="cursor-pointer">
                        {formData.accréditationOfficielle ? (
                          <div className="flex items-center justify-center gap-2">
                            <CheckCircle2 className="text-emerald-600" size={20} />
                            <span className="text-sm text-cacao">{formData.accréditationOfficielle.name}</span>
                          </div>
                        ) : (
                          <div>
                    ...
                    {errors.accréditationOfficielle && (
                      <p className="text-red-500 text-xs mt-1 flex items-center gap-1">
                        <AlertCircle size={12} /> {errors.accréditationOfficielle}
                      </p>
                    )}
                  </div>
                  ...
                  <div>
                    <label className="block text-sm font-bold text-cacao mb-2">
                      Certificat de compétences <span className="text-red-500">*</span>
                    </label>
                    <div className={`border-2 border-dashed rounded-xl p-4 text-center transition-all cursor-pointer hover:border-gold ${
                      errors.certificatCompétences ? 'border-red-300' : 'border-cacao/20'
                    }`}>
                      <input
                        type="file"
                        accept=".pdf,.jpg,.jpeg,.png"
                        onChange={(e) => e.target.files?.[0] && handleFileUpload('certificatCompétences', e.target.files[0])}
                        className="hidden"
                        id="competences"
                      />
                      <label htmlFor="competences" className="cursor-pointer">
                        {formData.certificatCompétences ? (
                          <div className="flex items-center justify-center gap-2">
                            <CheckCircle2 className="text-emerald-600" size={20} />
                            <span className="text-sm text-cacao">{formData.certificatCompétences.name}</span>
                          </div>
                        ) : (
                          <div>
                            <Upload className="w-8 h-8 text-cacao/40 mx-auto mb-2" />
                    ...
                    {errors.certificatCompétences && (
                      <p className="text-red-500 text-xs mt-1 flex items-center gap-1">
                        <AlertCircle size={12} /> {errors.certificatCompétences}
                      </p>
                    )}
                  </div>
                  ...
                  <div>
                    ...
                    <div className={`border-2 border-dashed rounded-xl p-4 text-center transition-all cursor-pointer hover:border-gold ${
                      errors.agrémentMinistériel ? 'border-red-300' : 'border-cacao/20'
                    }`}>
                      <input
                        type="file"
                        accept=".pdf,.jpg,.jpeg,.png"
                        onChange={(e) => e.target.files?.[0] && handleFileUpload('agrémentMinistériel', e.target.files[0])}
                        className="hidden"
                        id="agreement"
                      />
                      <label htmlFor="agreement" className="cursor-pointer">
                        {formData.agrémentMinistériel ? (
                          <div className="flex items-center justify-center gap-2">
                            <CheckCircle2 className="text-emerald-600" size={20} />
                            <span className="text-sm text-cacao">{formData.agrémentMinistériel.name}</span>
                          </div>
                        ) : (
                          <div>
                            <Upload className="w-8 h-8 text-cacao/40 mx-auto mb-2" />
                    ...
                    {errors.agrémentMinistériel && (
                      <p className="text-red-500 text-xs mt-1 flex items-center gap-1">
                        <AlertCircle size={12} /> {errors.agrémentMinistériel}
                      </p>
                    )}
                  </div>
                  ...
                  <div>
                    <label className="block text-sm font-bold text-cacao mb-2">
                      Photo du bureau <span className="text-red-500">*</span>
                    </label>
                    <div className={`border-2 border-dashed rounded-xl p-4 text-center transition-all cursor-pointer hover:border-gold ${
                      errors.photoBureau ? 'border-red-300' : 'border-cacao/20'
                    }`}>
                      <input
                        type="file"
                        accept=".jpg,.jpeg,.png"
                        onChange={(e) => e.target.files?.[0] && handleFileUpload('photoBureau', e.target.files[0])}
                        className="hidden"
                        id="photo"
                      />
                      <label htmlFor="photo" className="cursor-pointer">
                        {formData.photoBureau ? (
                          <div className="flex items-center justify-center gap-2">
                            <CheckCircle2 className="text-emerald-600" size={20} />
                            <span className="text-sm text-cacao">{formData.photoBureau.name}</span>
                          </div>
                        ) : (
                          <div>
                    ...
                    {errors.photoBureau && (
                      <p className="text-red-500 text-xs mt-1 flex items-center gap-1">
                        <AlertCircle size={12} /> {errors.photoBureau}
                      </p>
                    )}
                  </div>
                </div>
          ...
          >
            ...
            <div className="bg-cacao/5 rounded-2xl p-6 space-y-4">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4 text-sm">
                <div>
                  <span className="font-bold text-cacao/60">Organisme:</span>
                  <p className="text-cacao">{formData.nomOrganisme}</p>
                </div>
                <div>
                ...
                <div>
                  <span className="font-bold text-cacao/60">Type:</span>
                  <p className="text-cacao">{formData.typeOrganisme}</p>
                </div>
                <div>
                ...
                <div className="flex items-start gap-3">
                  ...
                  <div>
                    ...
                    <p className="text-xs text-amber-700">
                      ...
                      Le processus de validation peut prendre 5-7 jours ouvrés. 
          ...
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-4">
              <motion.button
                ...
                whileTap={{ scale: 0.95 }}
                onClick={() => navigate('/certifieur')}
                className="p-2.5 bg-white shadow-sm border border-cacao/10 rounded-xl text-cacao hover:text-gold transition-all"
              >
                <ArrowLeft size={20} />
              </motion.button>
              <div>
                <h1 className="text-xl font-serif font-black text-cacao tracking-tight">Inscription Certificateur</h1>
      ...
      <div className="bg-white border-b border-cacao/10">
        <div className="max-w-7xl mx-auto px-6 py-4">
          ...
          <div className="flex justify-between mt-2">
            <span className="text-xs text-cacao/60">Organisme</span>
