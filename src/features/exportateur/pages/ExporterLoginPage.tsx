...
import { QrCode, ArrowLeft, Loader2, ShieldCheck, X, Plane } from 'lucide-react';

type ExporterLoginFormData = {
  internalId: string;
  password: string;
};

const ExporterLoginPage: React.FC = () => {
  const navigate = useNavigate();
  const loginToStore = useAuthStore((state) => state.login);
  const [showScanner, setShowScanner] = useState(false);
  
  const { register, handleSubmit, formState: { errors, isSubmitting }, setValue } = useForm<ExporterLoginFormData>({
    defaultValues: {
      internalId: '',
      password: '',
    }
  ...
  const onSubmit = async (data: ExporterLoginFormData) => {
    try {
      ...
      <div className="w-full max-w-4xl bg-white md:rounded-4xl shadow-2xl overflow-hidden flex flex-col md:flex-row min-h-screen md:min-h-150">
        ...
        <div className="hidden md:flex md:w-1/2 bg-[#2B1608] p-12 flex-col justify-between text-white relative">
          ...
          <div className="relative z-10">
            ...
            <p className="mt-6 text-white/70 text-lg leading-relaxed">
              Espace d'exportation certifié pour les distributeurs de cacao.
            </p>
          </div>
        ...
        <div className="w-full md:w-1/2 p-8 md:p-12 flex flex-col">
          <div className="flex items-center justify-between mb-8">
            ...
            <span className="md:hidden font-black text-cacao">EXPORT PORTAL</span>
          </div>
          ...
          <div className="mb-8">
            ...
            <p className="text-gray-500 text-sm mt-1">Identifiez votre organisation d'exportation</p>
          </div>
          ...
          <form onSubmit={handleSubmit(onSubmit)} className="space-y-6">
            ...
            <div>
              ...
              <input 
                {...register("internalId")}
                placeholder="Ex: EXPORT-LOME-2026"
            ...
            <div>
              ...
              <input 
                ...
                className={`w-full p-4 bg-gray-50 rounded-xl border-2 transition-all outline-none font-bold ${
                  errors.password ? 'border-red-500 focus:border-red-500' : 'border-transparent focus:border-gold focus:bg-white'
                }`}
              />
          ...
          <div className="mt-auto pt-8 text-center">
            <p className="text-[9px] text-gray-400 uppercase tracking-[0.3em] font-black">Node d'Exportation #123 - Togo Network</p>
          </div>
        </div>
      </div>
    </div>
...
export default ExporterLoginPage;
